...
int mtfront_data_sent (connection_job_t c, int bytes_sent) {
  if (CONN_INFO(c)->type == &ct_tcp_rpc_ext_server_mtfront && bytes_sent > 0) {
    tcp_rpcs_note_connection_traffic (c, 0, bytes_sent);
  }
  // lru_insert_conn (c);
  return 0;
}
...
void mtfront_pre_loop (void) {
  ...
  if (!workers) {
    if (hot_admin_socket && tcp_rpcs_start_admin_server (hot_admin_socket, hot_admin_token) < 0) {
      kprintf ("cannot start hot-reload admin socket at %s: %m\n", hot_admin_socket);
      exit (1);
    }
...
int f_parse_option (int val) {
  ...
  case 2000:
    ...
    break;
  case 2010:
    hot_admin_socket = strdup (optarg);
    break;
  case 2011:
    hot_secrets_state_file = strdup (optarg);
    break;
  case 2012:
...
void mtfront_pre_init (void) {
  ...
  if (hot_secrets_state_file) {
    ...
    if (tcp_rpcs_load_secrets_state () < 0) {
      kprintf ("cannot load hot-reload secrets state from %s: %m\n", hot_secrets_state_file);
      exit (1);
    }
  }
  ...
  if (domain_count) {
    ...
    if (tcp_rpcs_active_secret_count (time (NULL)) == 0) {
